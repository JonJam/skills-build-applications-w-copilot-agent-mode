  ...
  return (
    <div className="card">
      <div className="card-header d-flex align-items-center">
        <h3 className="mb-0 me-auto">Leaderboard</h3>
            ...
            <tbody>
              {items.map((it, idx) => (
                <tr key={it.id || idx}>
                  <td>{idx + 1}</td>
                  <td>{it.id || '-'}</td>
                  <td>{it.user || it.name || `Entry ${idx + 1}`}</td>
                  <td><small className="text-muted">{renderJsonSnippet(it)}</small></td>
                  <td>
                    <button className="btn btn-outline-secondary btn-sm" onClick={() => { console.log('[Leaderboard] view', it); setSelected(it); }}>View</button>
                  </td>
                </tr>
              ))}
            </tbody>
